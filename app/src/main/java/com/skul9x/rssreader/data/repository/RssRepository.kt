...
class RssRepository(
    ...
    companion object {
        ...
        private const val READ_HISTORY_CLEANUP_DAYS = 7 // Keep read history for 7 days
    }
    ...
    /**
     * Get IDs of news items read today.
     */
    private suspend fun getTodayReadIds(): Set<String> {
        val todayStart = getTodayStartTimestamp()
        return readNewsDao.getTodayReadIds(todayStart).toSet()
    }
    ...
    suspend fun getRandomNewsFromCache(count: Int = 5): List<NewsItem> = withContext(Dispatchers.IO) {
        ...
        val cached = cachedNewsDao.getRandomNewsFromFeeds(feedIds, count * 3)
        val newsItems = cached.map { it.toNewsItem() }
        
        // Filter out already-read items
        val filtered = filterOutReadNews(newsItems)
    ...
    suspend fun seedDefaultFeedsIfEmpty() {
        ...
        val defaultFeeds = listOf(
            ...
            RssFeed(name = "VnExpress - Khoa học", url = "https://vnexpress.net/rss/khoa-hoc-cong-nghe.rss"),
            RssFeed(name = "Dân trí - Khoa học", url = "https://dantri.com.vn/rss/khoa-hoc.rss"),
            RssFeed(name = "Thanh Niên - Công nghệ", url = "https://thanhnien.vn/rss/cong-nghe.rss")
        )
