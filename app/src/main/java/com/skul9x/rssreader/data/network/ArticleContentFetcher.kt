...
class ArticleContentFetcher(
    ...
    suspend fun fetchArticleWithTitle(url: String, redirectCount: Int = 0): ArticleData? {
        ...
        // 1. Fetch HTML
        var (bytes, extractedCookie) = htmlFetcher.fetchHtmlWithCookieDetection(url, null)
        var html = bytes?.let { String(it, Charsets.UTF_8) }
        
        if (bytes == null || html == null) return null
        
        // 2. Anti-bot Retry (Cookie)
        if (extractedCookie != null && HtmlFetcher.RELOAD_PATTERN.containsMatchIn(html)) {
            logger.logEvent(
            ...
            val (retryBytes, _) = htmlFetcher.fetchHtmlWithCookieDetection(url, extractedCookie)
            val retryHtml = retryBytes?.let { String(it, Charsets.UTF_8) }
            if (retryBytes != null && retryHtml != null) {
                bytes = retryBytes
                html = retryHtml
            }
        }
        ...
        // 3.5. Special Redirects (Voz) - Check by Regex before massive Jsoup parse
        if (url.contains("voz.vn")) {
            val redirectUrl = redirectResolver.extractVozOriginalLinkRegex(html)
            if (redirectUrl != null && redirectUrl != url) {
                logger.logEvent(
                    eventType = "REDIRECT", 
                    url = url, 
                    message = "Phát hiện redirect từ Voz.vn (bằng Regex)", 
                    details = "Target: $redirectUrl",
                    isError = false
                )
                return fetchArticleWithTitle(redirectUrl, redirectCount + 1)
        ...
        // 4. Parse HTML with Auto-Charset detection via InputStream
        val doc = try {
        ...
        // 5. Fallback Voz features that require DOM 
        if (url.contains("voz.vn")) {
     ...
     */
    private suspend fun extractArticleFromHtml(bytes: ByteArray, url: String): ArticleData? {
    ...
    suspend fun analyzeRawHtml(html: String, mockUrl: String = "https://debug.local/article"): ArticleData? {
        return extractArticleFromHtml(html.toByteArray(Charsets.UTF_8), mockUrl)
    }
