...
class FirestoreSyncRepository private constructor(
    private var logRepo: com.skul9x.rssreader.data.repository.FirebaseLogRepository?
) {

    fun setLogRepo(repo: com.skul9x.rssreader.data.repository.FirebaseLogRepository) {
        this.logRepo = repo
    }
    ...
    companion object {
        ...
        fun getInstance(logRepo: com.skul9x.rssreader.data.repository.FirebaseLogRepository? = null): FirestoreSyncRepository {
            return INSTANCE ?: synchronized(this) {
                ...
                // Update logRepo if provided (fixes case where first caller passes null)
    ...
    suspend fun downloadSince(sinceTimestamp: Long): Pair<List<ReadNewsItem>, Long> {
        try {
            ...
            return Pair(items, maxTimestamp)
        } catch (e: Exception) {
            logRepo?.logError("Failed to download items since $sinceTimestamp", e)
            throw e // Rethrow to let coordinator handle failure
        }
    }
