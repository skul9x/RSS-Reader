...
class VnEconomyExtractor(private val htmlCleaner: HtmlCleaner) : SiteContentExtractor {
    ...
    override fun extractByJsoup(doc: Document, url: String): String? {
        ...
        workingDoc.select("script, style, iframe, .ads, .advertisement, .relate-container, .related-news, .box-keyword, .news-reference").remove()
        
        val content = StringBuilder()

        // Sapo
        val sapo = workingDoc.select(".news-sapo").text()
        if (sapo.isNotEmpty()) {
        ...
        if (textParts.isNotEmpty()) {
            content.append(textParts.joinToString("\n\n"))
        } else {
            // Fallback: get whole text if no paragraphs found
