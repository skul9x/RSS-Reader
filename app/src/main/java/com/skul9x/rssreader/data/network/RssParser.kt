...
class RssParser {
                ...
                when (eventType) {
                    ...
                    XmlPullParser.END_TAG -> {
                        ...
                        if (tagName == "item" || tagName == "entry") {
                            if (title.isNotBlank()) {
                                val cleanLink = link.normalizeNewsUrl()
                                items.add(
                                    NewsItem(
                                        id = generateId(cleanLink, title),
                                        title = title,
    ...
    /**
     * Extension to remove common tracking parameters from URLs.
     */
    private fun String.normalizeNewsUrl(): String {
        return this.substringBefore("?utm_")
                   .substringBefore("&utm_")
                   .substringBefore("?fbclid=")
                   .substringBefore("&fbclid=")
