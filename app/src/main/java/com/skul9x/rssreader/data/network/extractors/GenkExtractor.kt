...
class GenkExtractor(private val htmlCleaner: HtmlCleaner) : SiteContentExtractor {
    ...
    override fun extractByJsoup(doc: Document, url: String): String? {
        ...
        val sapo = doc.select(".knc-sapo, .detail-sapo, .sapo, h2.sapo").text()
        if (sapo.isNotEmpty()) content.append(sapo).append("\n\n")
        
        val body = doc.select("#ContentDetail, .knc-content, .detail-content, .singular-content, .cms-body, article .content").firstOrNull()
        if (body != null) {
