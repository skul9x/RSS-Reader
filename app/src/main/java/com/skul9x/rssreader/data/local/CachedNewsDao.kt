...
interface CachedNewsDao {
     ...
     */
    @Query("SELECT * FROM cached_news ORDER BY RANDOM() LIMIT :count")
    suspend fun getRandomNews(count: Int): List<CachedNewsItem>

    /**
     * Get random news from specific feeds.
     */
    @Query("SELECT * FROM cached_news WHERE feedId IN (:feedIds) ORDER BY RANDOM() LIMIT :count")
    suspend fun getRandomNewsFromFeeds(feedIds: List<Long>, count: Int): List<CachedNewsItem>
