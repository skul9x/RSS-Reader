...
class HtmlFetcher(private val logger: AppLogger) {
     ...
     */
    suspend fun fetchHtmlWithCookieDetection(url: String, cookie: String?): Pair<ByteArray?, String?> {
        return try {
            ...
            withContext(Dispatchers.IO) {
                response.use { resp ->
                    if (resp.isSuccessful) {
                        ...
                        val html = bytes?.let { String(it, Charsets.UTF_8) }
                        if (bytes != null && html != null && html.isNotBlank()) {
                            // Log HTML preview for debugging
                            val htmlPreview = html.take(200).replace("\n", " ").replace("\r", "")
                            logger.logEvent(
                                eventType = "HTML_RECEIVED",
                                url = url,
                                message = "Nhận HTML: ${bytes.size} bytes",
    ...
    suspend fun fetchRawHtml(url: String): String? {
        val (bytes, extractedCookie) = fetchHtmlWithCookieDetection(url, null)
        val html = bytes?.let { String(it, Charsets.UTF_8) }
        
        if (html != null && extractedCookie != null && RELOAD_PATTERN.containsMatchIn(html)) {
             // Anti-bot retry logic
             val (retryBytes, _) = fetchHtmlWithCookieDetection(url, extractedCookie)
             return retryBytes?.let { String(it, Charsets.UTF_8) }
        }
        return html
    }
}
